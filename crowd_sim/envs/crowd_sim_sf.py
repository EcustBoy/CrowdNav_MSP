...
class CrowdSim_SF(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, desired_vel=1, delta_t=0.25, tau=0.3, v0=100, sigma=0.3):
        """
        ...
        self.init_params = {'delta_t' : delta_t, 'sigma' : sigma, 'v0' : v0, 'tau' : tau}
    ...
    def humans_to_sf_state(self, humans, init=False) :
        
        def human_state(human, init) :
            if init :
                vx = human.gx - human.px
                vy = human.gy - human.py
                norm = (vx**2 + vy**2)**0.5
                vx = vx * self.desired_vel/(norm*1.3)
                vy = vy * self.desired_vel/(norm*1.3)
                return [human.px, human.py, vx, vy, human.gx, human.gy]
            
            return [human.px, human.py, human.vx, human.vy, human.gx, human.gy]
        
        return np.stack([human_state(human, init=init) for human in humans])
    
    def sf_state_to_humans(self, states) :
        
        def state_human(state) :
            human = Human(self.config, 'humans')
            human.set(state[0], state[1], state[4], state[5], state[2], state[3], 0)
            return human
        
        return [state_human(states[i]) for i in range(states.shape[0])]
    ...
    def reset(self, phase='test', test_case=None):
        ...
        self.simulator = socialforce.Simulator(self.humans_to_sf_state(self.humans, init=True), ped_ped=socialforce.PedPedPotential(self.delta_t, self.v0, self.sigma), delta_t=self.delta_t, tau=self.tau)
    ...
    def step(self, action, update=True):
        ...
        if update:
            ...
            self.states.append([self.robot.get_full_state(), [human.get_observable_state() for human in self.humans]])
            if hasattr(self.robot.policy, 'action_values'):
            ...
            new_state = self.simulator.step().state.copy()
            self.humans = self.sf_state_to_humans(new_state)
            self.global_time += self.time_step
            for i, human in enumerate(self.humans):
    ...
    def reset_domain(self) :
        self.modify_domain(self.init_params)
